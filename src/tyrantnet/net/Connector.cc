...
#include <tyrantnet/net/poller.h>
...
namespace tyrantnet { namespace net {
    ...
    void ConnectorWorkInfo::checkConnectStatus(int millsecond)
    {
        ...
        while (true)
        {
            ...
            totalFds.insert(fd);
            if (isConnectSuccess(fd, false) && !tyrantnet::net::base::IsSelfConnect(fd))
            {
    ...
    void ConnectorWorkInfo::checkTimeout()
    {
        for (auto it = mConnectingInfos.begin(); it != mConnectingInfos.end();)
        {
            ...
            tyrantnet::net::base::SocketClose(fd);
            if (cb != nullptr)
            {
    ...
    void ConnectorWorkInfo::processConnect(const AsyncConnectAddr& addr)
    {
        ...
        clientfd = tyrantnet::net::base::SocketCreate(AF_INET, SOCK_STREAM, 0);
        if (clientfd == INVALID_SOCKET)
        {
            goto FAILED;
        }

        tyrantnet::net::base::SocketNonblock(clientfd);
        ...
        if (n == 0)
        {
            if (tyrantnet::net::base::IsSelfConnect(clientfd))
            {
                goto FAILED;
            }
            else
    ...
    FAILED:
        if (clientfd != INVALID_SOCKET)
        {
            tyrantnet::net::base::SocketClose(clientfd);
    ...
    static void runOnceCheckConnect(const std::shared_ptr<tyrantnet::net::EventLoop>& eventLoop,
