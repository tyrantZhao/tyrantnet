...
EventLoop::EventLoop()
#ifdef PLATFORM_WINDOWS
TYRANT_NOEXCEPT : mIOCP(CreateIoCompletionPort(INVALID_HANDLE_VALUE, 0, 0, 1)), mWakeupChannel(new WakeupChannel(mIOCP))
#else
TYRANT_NOEXCEPT: mEpollFd(epoll_create(1))
#endif
{
#ifdef PLATFORM_WINDOWS
...
EventLoop::~EventLoop() TYRANT_NOEXCEPT
{
#ifdef PLATFORM_WINDOWS
    CloseHandle(mIOCP);
...
::tyrant::TimerMgr::PTR EventLoop::getTimerMgr()
{
    tryInitThreadID();
